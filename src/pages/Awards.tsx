    ...
    <PageTransition>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-zinc-500">Scholarships, competitions, and academic honors.</p>
        </div>
          ...
          {sortedAwards.map((award) => (
            <Card key={award.id} className="p-6 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <div className="w-10 h-10 rounded-lg bg-amber-900/20 text-amber-500 flex items-center justify-center">
                  <Trophy className="w-6 h-6" />
                </div>
                <div className="text-xs font-bold text-zinc-400 flex items-center">
              ...
              {(award.associatedProjects || award.associatedPublications) && (
                <div className="pt-4 border-t border-zinc-800 flex flex-wrap gap-3">
                  {award.associatedProjects?.map(projId => (
                    <Link key={projId} to={`/projects?id=${projId}`} className="text-xs font-bold text-zinc-500 hover:text-primary-400 flex items-center">
                      Related Project <ExternalLink className="w-3 h-3 ml-1" />
                    </Link>
                  ))}
                  {award.associatedPublications?.map(pubId => (
                    <Link key={pubId} to={`/publications/${pubId}`} className="text-xs font-bold text-zinc-500 hover:text-primary-400 flex items-center">
